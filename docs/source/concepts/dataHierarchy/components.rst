...
Plugin Components
^^^^^^^^^^^^^^^^^
These are user-defined component classes that can load or generate data for specific software, hardware, or speacial use cases.

Helper Components
^^^^^^^^^^^^^^^^^

These are component classes of your ``DataUnit`` that aid in proccessing. The current list of helper components ``dmanage`` provides are
