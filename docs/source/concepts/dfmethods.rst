...
D-Manage has a subpackage ``dmanage.dfmethods`` with common processing methods for properly formated DataFrames! These are community developed methods that use DataFrame input and process data. Many of the methods are just wrappers for common functions provided by other Python packages such at ``scipy``, ``numpy``, and ``matplotlib``. 
