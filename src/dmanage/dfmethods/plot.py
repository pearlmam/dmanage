...
def check_fig(fig, figsize, clear, projection='rectilinear', subplots=(1, 1)):
    # get the proper figure ref
    if type(fig) == type(None): fig = plt.figure(Defs.fig, figsize=figsize)
...
def plot1d(DF, fig=None, figsize=(12, 5), clear=True, subplots=(1, 1), subplot=0, axType='linear', drawFig=True, convertAxis=True, **line2Dkwargs):
    ...
    fig,axs = check_fig(fig, figsize, clear, projection, subplots=subplots)
    ...
    if convertAxis:
        x,xlabel = Defs.convert_axis(DF.index.name, x)
...
def plot1ds(DF, fig=None, figsize=(12, 5), clear=True, drawFig=False, **line2Dkwargs):
    ...
    fig,ax = check_fig(fig, figsize, clear)

    DF = DF.unstack(iNames[0])[colName]
    fig,ax = plot1d(DF, fig=fig, figsize=figsize, clear=clear, drawFig=False, **line2Dkwargs)
...
def bar(DF, fig=None, figsize=(12, 5), clear=True, convertAxis=True, **line2Dkwargs):
    if type(DF.index) == pd.core.indexes.multi.MultiIndex: DF = mi_to_index(DF)
    if issubclass(type(DF), pd.core.series.Series): DF = DF.to_frame()
    if len(DF.columns)>1: raise Exception("DF must be of type Series or of type DataFrame with one column.")
...
def num_scatter(DF, fig=None, figsize=(12, 5), clear=True, convertAxis=True, **line2Dkwargs):
    if type(DF.index) == pd.core.indexes.multi.MultiIndex: DF = mi_to_index(DF)
    if issubclass(type(DF), pd.core.series.Series): DF = DF.to_frame()
    ...
    for i,col in enumerate(DF.columns):
        ...
        for j in range(0,len(y)):
            plt.text(x[j], y[j], j, fontsize=Defs.tickfs, horizontalalignment='center', verticalalignment='bottom')
    return fig,ax


def prep_text_chart(DF, fmt=".2f"):
    ...
    for i,col in enumerate(DF.columns):
        if col in Defs.axisUnits.keys():
            unit = Defs.axisUnits[col]
            cols[i] = '%s\n%s'%(col,unit)
            if unit in Defs.unitFactors.keys():
                DF[col] = DF[col]*Defs.unitFactors[unit]
    DF.columns = cols
...
def num_scatter_chart(DF, fmt=[".0f", '.2f', '.1f'], textPos=None, fontsize=14, fig=None, figsize=(12, 5), clear=True):
    if type(DF.index) == pd.core.indexes.multi.MultiIndex: DF = mi_to_index(DF)
    if issubclass(type(DF), pd.core.series.Series): DF = DF.to_frame()
    if len(DF.columns)>1: raise Exception("DF must be of type Series or of type DataFrame with one column.")
...
def scatter(DF, fig=None, figsize=(12, 5), clear=True, subplots=(1, 1), subplot=0, drawFig=True, convertAxis=True, axType='linear', **line2Dkwargs):
...
def scatter_color(DF, fig=None, figsize=(12, 5), clear=True, cbar=True, subplots=(1, 1), subplot=0, drawFig=True, convertAxis=True, **line2Dkwargs):
    """
...
def tricontourf(DF, fig=None, figsize=(12, 5), clear=True, cmap='viridis', convertAxis=True):
...
def contourf(DF, fig=None, figsize=(12, 5), clear=True, polar=False, cmap='viridis', convertAxis=True):
    ...
    fig,ax = check_fig(fig, figsize, clear, projection)
...
def plot1d_slider(DF, fig=1, clear=True):
    ...
    fig,ax = check_fig(fig=fig, figsize=(12, 5), clear=clear)
...
def plot1d_slider2(DF, plotVar=False, fig=1, fmt='#e', clear=True):
    if not issubclass(type(DF), pd.core.series.Series):
...
def interact_tagged_paricles(DF, tags=None, fig=None, figsize=(8, 8), clear=True, freq=None):
    xy = ['x','y']
    projection='rectilinear'
    fig,ax = check_fig(fig, figsize, clear, projection)
    if tags is None:
...
def labeled_scatter(DF, labelCol=None, fig=None, figsize=(12, 5), clear=True, subplots=(1, 1), subplot=0, convertAxis=True, axType='linear', **line2Dkwargs):
    if type(DF.index) == pd.core.indexes.multi.MultiIndex: DF = mi_to_index(DF)
    if issubclass(type(DF), pd.core.series.Series): DF = DF.to_frame()
    ...
    fig,ax = scatter(DF[columns],fig=fig,figsize=figsize,clear=clear,subplots=subplots,subplot=0,convertAxis=convertAxis,axType=axType,**line2Dkwargs)
    ...
    for i,col in enumerate(columns):
        ...
        for j,label in enumerate(DF[labelCol]):
            plt.text(x[j], y[j], label, fontsize=Defs.tickfs, horizontalalignment='center', verticalalignment='bottom')
    return fig,ax


def plot1d_pks(DF, fig=None, figsize=(12, 5), subplots=(1, 1), subplot=0, maxPks=10, hRatio=None, pRatio=None, height=None, xlim=None):
    if type(DF.index) == pd.core.indexes.multi.MultiIndex: DF = mi_to_index(DF)
    if issubclass(type(DF), pd.core.series.Series): DF = DF.to_frame()
    if len(DF.columns)>1: raise Exception("DF must be of type Series or of type DataFrame with one column.")
    ...
    DFpks,props = find_pks(DF,maxPks=maxPks,hRatio=hRatio,pRatio=pRatio,height=height)
    fig,ax = plot1d(DF, fig=fig)
    ax.set(xlim=xlim)
    fig,ax = num_scatter_chart(DFpks, fig=fig, clear=False)
    fig = draw_fig(fig)
    return fig,axs
...
def draw_fig(fig=1, option='no options availiable'):
    if not mpl.get_backend().lower() == 'agg':
