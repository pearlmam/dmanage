    ...
    class SweepDir(base):
        ...
        def _getDDs(self,subDirs,baseDir):
            ...
            for root in subDirs:
                ...
                if self.isValid(root) and not skip: 
                    sweepDirs.append(root.replace(baseDir,''))
                else: 
                    pass
        ...
        def getDDs(self,baseDir=None,nc=1):
            ...
            # print(type(subDirs))
            if nc > 1:
                variables = [(subDir,baseDir) for subDir in subDirs]
                pool = Pool(processes=nc)
                result = pool.map_async(self._getDDs,variables)
                result.wait()
                sweepDirsList = result.get()
                pool.close()
                sweepDirs = [x for l in sweepDirsList for x in l]  
            else:
                sweepDirs = self._getDDs(subDirs,baseDir)
