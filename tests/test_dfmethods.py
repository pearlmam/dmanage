...
df1 = dmanage.compute.backends.dfmethods.signal.apply_filter(df0, method='low', cutoff=100e6)
...
startTime = time.time()
df1 = dmanage.compute.backends.dfmethods.convert.cart_to_cyl(df0, xyCols=['x', 'y'], uxyCols = ['ux', 'uy'], rphiCols=['r', 'phi'], phiRange='2pi', nc=1)
